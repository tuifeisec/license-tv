...
**File:** `PlanTypeBadge.tsx`
**Props:** `value: PlanType`
**Usage:** 套餐类型标签，统一 monthly / quarterly / yearly / lifetime / trial 的视觉展示。
...
**Usage:** 页面标题区，保持控制台顶部标题与操作区的统一结构。

### DataTable

**File:** `DataTable.tsx`
**Props:** `data`, `columns`, `keyExtractor`, `mobileRender?`, `empty?`, `onRowClick?`
**Usage:** 列表页统一表格组件，桌面端渲染表格，移动端自动切换卡片列表。

### TVUsernameInput

**File:** `TVUsernameInput.tsx`
**Props:** `value`, `onChange`, `validator`, `placeholder?`, `hintLabel?`
**Usage:** TradingView 用户名输入与实时校验组合组件，适用于新建客户和提交申请。

### ScriptSelect

**File:** `ScriptSelect.tsx`
**Props:** `value`, `onChange`, `scope?`, `placeholder?`
**Usage:** 从 API 动态加载脚本列表的选择组件，支持 admin / agent 两种作用域。

### PlanTypeSelect

**File:** `PlanTypeSelect.tsx`
**Props:** `value`, `onChange`
**Usage:** 套餐类型标准下拉选择器，避免业务页重复手写选项。

### CookieStatusCard

**File:** `CookieStatusCard.tsx`
**Props:** `data: TVSessionStatus`
**Usage:** 系统设置页展示 TV Cookie 配置与连接有效性。

### AuditReportTable

**File:** `AuditReportTable.tsx`
**Props:** `entries: AccessAuditEntry[]`
**Usage:** 授权对账结果表格，展示脚本维度的 TV / DB 不一致项。
...
**File:** `ScriptStatusToggle.tsx`
**Props:** `checked`, `onCheckedChange`, `pending?`, `disabled?`, `className?`
**Usage:** Compact enable or disable control for script management screens. Keeps script status operations consistent across the list and detail views.

```tsx
<ScriptStatusToggle checked onCheckedChange={setChecked} />
```
