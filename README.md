...
- 仓库只提交 `config/config.example.yaml`
- 不要提交真实的 `config/config.yaml`
- 正式环境必须替换 `jwt.secret`、管理员密码、TradingView 会话信息和加密密钥
- 上传 GitHub 前确认本地日志、上传目录、缓存目录未被强制提交
- 如果仓库准备公开，建议进一步清理文档中的示例账号、示例密码和内部业务术语
...
可以公开代码，但在公开前建议先检查配置文件、文档示例值、业务命名和内部部署信息是否需要进一步脱敏。
...
- 上传仓库或交接代码前，先确认本地配置、日志、缓存和构建产物没有混入提交范围

## GitHub 上传前检查

上传前建议确认：

- `.codex/`、`.cursor/`、`node_modules/`、`dist/`、`runlogs/`、`uploads/` 已忽略
- 真实配置文件 `config/config.yaml` 不会被提交
- 文档中的示例账号或示例密码是否需要进一步脱敏
- 不要将本地推送脚本、缓存目录或临时构建文件强制加入 Git
